...
void URswFunctionLibrary::ServerTravel(const UObject* WorldContextObject, TSoftObjectPtr<UWorld> Level)
{
	FString Path = Level.ToSoftObjectPath().ToString();
	int32 DotIndex;
	if (Path.FindChar('.', DotIndex))
	{
		Path = Path.Left(DotIndex); // È¥µô .MyMap ºó×º
	}

	WorldContextObject->GetWorld()->ServerTravel(Path + TEXT("?listen"));
}
