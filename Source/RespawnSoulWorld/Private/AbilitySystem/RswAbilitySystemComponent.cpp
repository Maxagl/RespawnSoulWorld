...
{
    if (!InInputTag.IsValid() || !InInputTag.MatchesTag(RswGameplayTags::InputTag_MustBeHeld) && !InInputTag.MatchesTag(RswGameplayTags::InputTag_SpecialCharacterAbility_Charge))
    {
        return;
    }
    for (const FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
    {
        if (InInputTag.MatchesTag(RswGameplayTags::InputTag_SpecialCharacterAbility_Charge) && AbilitySpec.DynamicAbilityTags.HasTagExact(InInputTag) && AbilitySpec.IsActive())
        {
            FGameplayEventData PayLoad{};
            UAbilitySystemBlueprintLibrary::SendGameplayEventToActor(GetOwnerActor(), RswGameplayTags::Player_Event_Released_ChargeKey, PayLoad);
        }
        else if (AbilitySpec.DynamicAbilityTags.HasTagExact(InInputTag) && AbilitySpec.IsActive())
        {
            CancelAbilityHandle(AbilitySpec.Handle);
        }
    }
