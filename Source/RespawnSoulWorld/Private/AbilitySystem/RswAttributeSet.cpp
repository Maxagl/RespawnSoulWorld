...
void URswAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
    ...
    {
        // Debug::Print(FString("I am real life, clamp"));
    ...
    if (Data.EvaluatedData.Attribute == GetDamageTakenAttribute())
    {
        ...
        // 设置延迟 N 秒后更新延迟血条
        GetWorld()->GetTimerManager().SetTimer(DelayTimerHandle, [this, PawnUIComponent]()
            {
                this->UpdateDelayedHealth(PawnUIComponent);
            }, 1.0f, false);

        // Debug::Print(FString::SanitizeFloat(GetWorld()->GetTimeSeconds() - PassedTime));
        PassedTime = GetWorld()->GetTimeSeconds();
        // Debug::Print(FString("I am damaged"));
...
void URswAttributeSet::UpdateDelayedHealth(UPawnUIComponent* PawnUIComponent)
{
    GreyHealth = CurrentHealth;
    PawnUIComponent->OnGreyHealthChanged.Broadcast(GetGreyHealth() / GetMaxHealth());
}
