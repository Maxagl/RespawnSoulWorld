...
// 执行的时候设置NodeMemory
// Called once when the task node is executed.
...
// Called every frame while the task is running and still marked as InProgress
void UBTTask_RotateToFaceTarget::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	else
	{
		...
		FRotator TargetRot = FMath::RInterpTo(Memory->OwningPawn->GetActorRotation(), LookAtRot, DeltaSeconds, RotationInterpSpeed);
		// 防止pitch发生变化，不然角色会pitch角度动了
		TargetRot.Pitch = 0;
